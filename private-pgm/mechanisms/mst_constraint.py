...
def MST(data, epsilon, delta, protected, outcome, admissible, inadmissible):
  ...
  cliques = select(data, rho / 3.0, log1, 
                   proc_attrs=protected, target_attrs=outcome,
...
def select(data, rho, measurement_log, 
           proc_attrs=['sex'], target_attrs=['income'], 
           admissible=['x'], inadmissible=['g'], cliques=[]):
    ...
    sorted_by_values = sorted(weights.items(), key=lambda item: item[1])
    for (a, b), w in sorted_by_values:
        print(
            "[INFO] Weight for edge ({}, {}): {}".format(
                a,
                b,
                weights[a, b],
            )
    ...
    def is_bad_path(graph, proc, target, admissible):
        """
        Returns True if there is a path from proc to target
        that does NOT pass through any admissible attributes.
        """
        if not nx.has_path(graph, proc, target):
            return False
        for path in nx.all_simple_paths(graph, source=proc, target=target):
            if not any(node in admissible for node in path):
                return True
        return False

    for i in range(r - 1):
        # Remove edges that connect already-connected nodes
        candidates = [e for e in candidates if not ds.connected(*e)]

        # Enforce fairness constraint
        filtered_candidates = []
        for e in candidates:
            # Tentatively add edge
            T.add_edge(*e)
            if any(is_bad_path(T, p, t, admissible) for p in proc_attrs for t in target_attrs):
                # If bad path created, discard
                print(f"[INFO] bad path detected at {i}")
                print(f"the path: {e}", end=' ')
                print("")
                T.remove_edge(*e)
            else:
                filtered_candidates.append(e)
                T.remove_edge(*e)  # remove tentative edge

        if not filtered_candidates:
            break  # no valid edges left, stop

        wgts = np.array([weights[e] for e in filtered_candidates])
        idx = exponential_mechanism(wgts, epsilon, sensitivity=1.0)
        e = filtered_candidates[idx]
        print(f"[INFO] Adding edge {e} at iteration {i}")
        T.add_edge(*e)
        ds.merge(*e)
    print("[INFO] Final edges in the tree:", list(T.edges))
    return list(T.edges)
