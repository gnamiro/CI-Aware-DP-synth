...
def MST(data, epsilon, delta, cmi_value, degree, proc_attr):
  rho = cdp_rho(epsilon, delta)
  sigma = np.sqrt(3 / (2 * rho))
  cliques = [(col,) for col in data.domain]
  log1 = measure(data, cliques, sigma)
  data, log1, undo_compress_fn = compress_domain(data, log1)
  cliques = select(data, rho / 3.0, log1, proc_attr=proc_attr)

  log2 = measure(data, cliques, sigma)
  est, logs = estimation.proximal_mirror_descent(data.domain, log1+log2, iters=30000, cmi_ratio = cmi_value, degree=degree)

  return est, undo_compress_fn, logs


def custom_weights(cliques, attr, low_weight=0.1, high_weight=1.0):
  raw_weights = []
  for clique in cliques:
    if attr in clique:
      raw_weights.append(low_weight)
    else:
      raw_weights.append(high_weight)
    
  raw_weights = np.array(raw_weights)
  return raw_weights / np.linalg.norm(raw_weights)

def measure(data, cliques, sigma, weights=None, o_w=None):
  if weights is None:
    weights = np.ones(len(cliques))

  if o_w is None:
    o_w = np.ones(len(cliques))

  weights = np.array(weights) / np.linalg.norm(weights)
  # o_w = np.array(o_w) / np.linalg.norm(o_w)
  measurements = []
  for proj, wgt in zip(cliques, weights):
    x = data.project(proj).datavector()
    y = x + np.random.normal(loc=0, scale=sigma / wgt, size=x.size)
    measurements.append(LinearMeasurement(y, proj, sigma / wgt, o_w[cliques.index(proj)]))
  return measurements
...
def select(data, rho, measurement_log, proc_attr, cliques=[]):
  ...
  for a, b in candidates:
    coeff = 1 if proc_attr not in (a, b) else 0.1
    xhat = est.project([a, b]).datavector()
    x = data.project([a, b]).datavector()
    weights[a, b] = np.linalg.norm(x - xhat, 1) * coeff
