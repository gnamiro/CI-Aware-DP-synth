...
class FactoredInference_reg:
    ...
    def estimate(self, measurements, total = None, engine='MD', callback=None, options = {}):
        ...
        """
        measurements = self.fix_measurements(measurements) # check the characteristcs of the info inside measurements
    ...
    def mirror_descent(self, measurements, total = None, stepsize = None, callback=None):
        ...
        if lambda_main* ans[0] + lambda_reg*reg_loss == 0:
            return lambda_main* ans[0] + lambda_reg*reg_loss
        ...
        for t in range(1, 3*self.iters + 1):
            ...
            if use_regularizer:
                ...
                reg_grad = self._numerical_reg_gradient_spsa(theta, epsilon)
    ...
    def proximal_mirror_descent(self, measurements, total = None, stepsize = None, callback=None):
        ...
        # Model potentials Random Initialization
        # model_potentials = {}
        # for cl in theta:
        #     # Generate random perturbation vector
        #     delta = (np.random.choice([1, -1], size=theta[cl].values.shape) * 1e2)
        #     theta[cl].values += delta
        ...
        for t in range(1, 2*self.iters + 1):
            ...
            if callback is not None:
                callback(mu)
            # lambda_reg = 3*1e10
            lambda_reg = min(15, t/200)

            # lambda_reg = 0
            marg_grad_norm = np.sqrt(ans[1].dot(ans[1]))
            # max_reg_norm = 1e-3 * marg_grad_norm
            marg_grad = ans[1]
    ...
    def _numerical_reg_gradient_spsa(self, theta, epsilon=1e-3, perturbation_scale=1e-2):
        ...
        for cl in theta:
            ...
            # print(f"for this cl:")
            # print(cl)
            # print(f"the generated delta is (perturbation scale is {perturbation_scale})")
            # print(delta)
            # print("**"*8)
            ...
            reg_loss_pos = self.calculate_conditional_mutual_information(theta_pos, self.constraint)
            
            # mu_neg = self.model.belief_propagation(theta_neg)
            reg_loss_neg = self.calculate_conditional_mutual_information(theta_neg, self.constraint)
    ...
    #     # Compute marginals for the projections
    ...
    #     if debugging:
    #         print(f"probability Z")
    #         print(prob_Z)
    #         print(f"probability XZ")
    #         print(prob_XZ)
    #         print(f"probability YZ")
    #         print(prob_YZ)
    #         print(f"probability XYZ")
    #         print(prob_XYZ)
    #     # Initialize CMI
    #     cmi = 0.0

    #     # Loop over all possible values of X, Y, and Z dimensions
    #     for x_idx in np.ndindex(prob_XYZ.shape[:len(constraint[0])]):
    #         for y_idx in np.ndindex(prob_XYZ.shape[len(constraint[0]):len(constraint[0]) + len(constraint[1])]):
    #             for z_idx in np.ndindex(prob_Z.shape):
    #                 # print(x_idx, y_idx, z_idx)
    #                 p_xyz = prob_XYZ[x_idx + y_idx + z_idx]
    #                 p_xz = prob_XZ[x_idx + z_idx]
    #                 p_yz = prob_YZ[y_idx + z_idx]
    #                 p_z = prob_Z[z_idx]
