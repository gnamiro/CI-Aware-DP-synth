...
class GraphicalModel_clean:
    ...
    def krondot(self, matrices):
        ...
        logZ, _ = self.belief_propagation(self.potentials, logZ=True)
    ...
    def calculate_many_marginals(self, projections):
        ...
        self.marginals, _ = self.belief_propagation(self.potentials)
        sep = self.sep_axes
    ...
    def belief_propagation(self, potentials, logZ=False):
        """ Compute the marginals and their derivatives w.r.t. theta
        ...
        :return d_beliefs: the derivative of the belief_propagation with respect to the theta
        """
        from mbi import Factor
        from mbi.torch_factor import Factor
        beliefs = {cl: potentials[cl].copy() for cl in potentials}
        messages = {}
        intermediate = {'beliefs': [], 'messages': [], 'seps': [], 'message_order': []}  # For storing intermediate results
        
        # Initialize derivatives (zero initialization)
        d_beliefs = {cl: Factor.zeros(pot.domain) for cl, pot in potentials.items()}
        d_messages = {}
        ...
        for i, j in self.message_order:
            ...
            intermediate['beliefs'].append(beliefs.copy())
            intermediate['messages'].append(messages.copy())
            intermediate['seps'].append(sep)
            intermediate['message_order'].append((i, j))
        
        print("-------------<><><><<><>><><>-------------")
        print("intermediate information: ")
        print(intermediate)

        # Compute partition function logZ if requested
        cl = self.cliques[0]
        logZ = beliefs[cl].logsumexp() if logZ else None

        # Backward pass (reverse the message order)
        for (i, j), sep in zip(reversed(intermediate['message_order']), reversed(intermediate['seps'])):
            print("-------------00000000000000000-------------")
            print(d_messages)
            print(i,j)
            print(sep)
            tau = intermediate['beliefs'].pop()[j]
            d_tau = d_beliefs[j]
            print(f"d_tau: {d_tau.domain.attrs} - {d_tau.domain.shape}")
            
            # Compute derivative of the message using chain rule
            exp_tau = tau.exp()
            sum_exp_tau = exp_tau.sum(attrs=j)
            d_messages[(i, j)] = (d_tau * exp_tau / sum_exp_tau).sum(attrs=j)
            print(f"exp_tau: {exp_tau.domain.attrs} - {exp_tau.domain.shape}")
            # Update the derivative beliefs
            print("-------------888888888-------------")
            print(d_beliefs[i].domain, d_messages[(i, j)].domain)
            d_message_projected = d_messages[(i, j)].project(d_beliefs[i].domain)

            # Update the derivative beliefs with the reindexed message
            d_beliefs[i] += d_messages[(i, j)]

        if logZ:
            d_logZ = d_beliefs[cl].logsumexp()
            for cl in self.cliques:
                beliefs[cl] += np.log(self.total) - logZ
                beliefs[cl] = beliefs[cl].exp(out=beliefs[cl])
                # Adjust derivatives accordingly
                d_beliefs[cl] = d_beliefs[cl] * np.exp(np.log(self.total) - logZ) - d_logZ

        return CliqueVector(beliefs), CliqueVector(d_beliefs)

    def backprop_through_belief_propagation(self, potentials, grad_mu, intermediate):
        """
        Perform backpropagation through belief propagation to compute the gradient
        with respect to the graphical model parameters (potentials).
        
        :param potentials: The original potentials (parameters) used in belief propagation
        :param grad_mu: The gradient of the loss function with respect to the marginals (mu)
        :return: The gradient with respect to the potentials
        """
        from mbi import Factor
        from mbi.torch_factor import Factor
        # Initialize gradients for potentials

        grad_potentials = CliqueVector({cl: Factor.zeros(pot.domain) for cl, pot in potentials.items()})
        grad_beliefs = grad_mu.copy()

        print("Poteintals Grad structure: ")
        print(grad_potentials)
        print("---------------------------<><><><><><><><>---------------------------")
        
        # Backward pass through the message-passing operations
        for step in reversed(range(len(self.message_order))):
            i, j = self.message_order[step]
            sep = intermediate['seps'][step]
            messages = intermediate['messages'][step]
            beliefs = intermediate['beliefs'][step]
            
            print("---------------------------<><><><><><><><>---------------------------")
            print("Before projection:", grad_beliefs[j].domain.attrs)
            print("Separator:", sep)
            print("After projection:", grad_beliefs[j].project(sep).domain.attrs)
            print("---------------------------<><><><><><><><>---------------------------")
            # Compute gradient w.r.t. messages (j, i)
            grad_msg = grad_beliefs[j].project(sep).expand(beliefs[i].domain)
            
            # Update grad_beliefs[i]
            grad_beliefs[i] += grad_msg
            
            # Update gradient w.r.t. potential[i]
            grad_potentials[i] += grad_beliefs[i]
            
            # Message passing update for grad_beliefs[j]
            if (j, i) in messages:
                grad_beliefs[i] -= messages[(j, i)]
        
        return grad_potentials
