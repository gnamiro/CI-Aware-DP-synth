...
class Factor:
    ...
    def project(self, attrs, agg = 'sum'):
        ...
        # print(f"torch_factor::79 marginalized version of --> {attrs}:")
        # print(marginalized)
        if agg == 'sum':
    ...
    def logsumexp(self, attrs = None):
        if attrs is None:
            ...
            return self
        axes = self.domain.axes(attrs)
        # print(f"103::torch_factor:: logsumexp-->")
        # print(self.values)
    ...
    def dataprobs(self, flatten=True):
        """ Materialize the probabilities as a numpy array """
        ans = self.values.to("cpu").numpy()  
        total_count = ans.sum()             
        prob_array = ans / total_count      
        
        return prob_array.flatten() if flatten else prob_array
