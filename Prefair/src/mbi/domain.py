...
class Domain:
    def __init__(self, attrs, shape, drop_cols=None):
        ...
        if drop_cols is not None:
            new_attrs = [attr for attr in attrs if attr not in drop_cols]
            new_shape = [shape[i] for i, attr in enumerate(attrs) if attr not in drop_cols]

        assert len(attrs) == len(shape), 'dimensions must be equal'
        self.attrs = tuple(new_attrs)
