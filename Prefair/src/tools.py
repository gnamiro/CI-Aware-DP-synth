...
def reduce_domain_size(df):
    ...
    # df['bucketized-marital-status'] = df['marital-status'].apply(lambda x: 'Never-married' if x == 'Never-married' else 'Married' if 'Married' in x else 'Seperated')
...
def OR(df,att,Y_features,X_features,inadmisable,priv,minor,positive=1):
    ...
    for adm_set in sub_sets:
        ...
        for name, group in grouped:
            ...
            # Drop rows with indices greater than 2
            count_filtered = count[count.index <= 2]
...
def binarize_attribute(df, att):
    return df[df[att] < 2]
    # return new_df

def OR2(df,att,Y_features,X_features,inadmisable,priv,minor,positive=1):
    sub_sets = list_of_combs(inadmisable)
    dood = 1.1
    df = binarize_attribute(df, att)
    for adm_set in sub_sets:
        grouped=df.groupby(X_features+adm_set)
        total=len(df.index)
        total=0
