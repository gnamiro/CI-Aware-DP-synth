import numpy as np
from mbi import FactoredInference_torch, FactoredInference_NO_NOISE, Dataset, Domain
...
def MST(data):
    ...
    cliques = [(col,) for col in data.domain] # 1-way marginals
    log1 = measure(data, cliques)
    data, log1, undo_compress_fn = compress_domain(data, log1)

    log1 = measure(data, cliques) # 1 way marginals values selected 
    # # cliques = tuple([col for col in data.domain])
    # # log_all = measure(data, [cliques])
    # # data, log_all, undo_compress_fn = compress_domain(data, log_all)
    cliques = select(data, log1) # 2-way marginals
    print(cliques)
    # cliques = [('sex', 'marital-status'), ('age', 'marital-status'), ('income', 'marital-status'), ('income', 'occupation'), ('occupation', 'education-num'), ('occupation', 'hours-per-week')]
    log2 = measure(data, cliques) # inject noise
    
    # cliques = [col for col in data.domain]
    # print(cliques)
    # print(len(cliques))
    # log_all = measure(data, [tuple(cliques)])

    # engine = FactoredInference_torch(data.domain, constraint=independence_columns, iters=5000)
    engine = FactoredInference_NO_NOISE(data.domain, iters=5000)
    # print("Lets print the information that is going to pass to the inference module--")
    # print(log1+log2)
    # print("--------------------------------------------------------------------------=--")
    # est = engine.estimate(log1+log2, engine='PMD',  reg_method='TVD', tv_method='L1')
    est = engine.estimate(log1+log2)

    synth = est.synthetic_data()
    # synth = est.synthetic_data(rows=int(1e5))
...
def select(data, measurement_log, cliques=[]):
    # print("Using one-way marginals to find 2-way marginals")
    # for Q, y, proj in measurement_log:
        # print("************************************")
        # print("Now checking the values for the measures")
        # print("Queries")
        # print(Q)
        # print("----------------")
        # print("answers ")
    ...
    # engine = FactoredInference_torch(data.domain, constraint=None, iters=1000)
    ...
    for a, b in candidates:
        ...
        xhat_prob = xhat / xhat.sum()
        a_marginal = data.project([a]).datavector().reshape(-1, 1)
        b_marginal = data.project([b]).datavector().reshape(1, -1)
        a_prob = a_marginal / a_marginal.sum()
        b_prob = b_marginal / b_marginal.sum()
        a_b_margin = np.dot(a_prob,b_prob).reshape(1, -1).squeeze()
        assert np.allclose(a_b_margin, xhat_prob, atol=1e-8) , f'1 way marginal is something else {a_b_margin} -- {xhat_prob}'
...
def calculate_conditional_mutual_information(dataset, constraint):
    ...
    # Convert to DataFrame for merging and broadcasting
    ...
    # Merge projections on shared attributes (Z)
    ...
    # Fill NaN with small epsilon to avoid division by zero
    epsilon = 1e-12
    ...
    # Sum over all contributions and ensure non-negative
