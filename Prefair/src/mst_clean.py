import numpy as np
from mbi import FactoredInference_torch, Dataset, Domain
...
def MST(data, epsilon, delta):
    ...
    cliques = select(data, rho/3.0, log1) # 2-way marginals
    print("Printing projection on the joint probability")
    print(cliques)
    print("-------------------------------------------------")
    log2 = measure(data, cliques, sigma) # inject noise
    engine = FactoredInference_clean(data.domain, iters=5000)
...
def select(data, rho, measurement_log, cliques=[]):
    engine = FactoredInference_clean(data.domain, iters=1000)
